...
def get_answer(
    ...
    if mode == "WEB搜索问答":
        for resp, history in local_doc_qa.get_search_result_based_answer(
            ...
            source += "".join(
                [
                    f"""<details> <summary>出处 [{i + 1}] <a href="{doc.metadata["url"]}" target="_blank">{doc.metadata["source"]}</a> </summary>\n"""
                    f"""{doc.page_content}\n"""
                    f"""</details>"""
        ...
        if os.path.exists(vs_path):
            ...
            if not resp["source_documents"]:
                yield history + [
                    [query, "根据您的设定，没有匹配到任何内容，请确认您设置的知识相关度 Score 阈值是否过小或其他参数是否正确。"]
                ], ""
            else:
                source = "\n".join(
                    [
        ...
        for resp, history in local_doc_qa.llm._call(
            query, history, streaming=streaming
        ):
            history[-1][-1] = resp + (
    ...
    if local_doc_qa.llm and local_doc_qa.embeddings:
        if isinstance(files, list):
            ...
            vs_path, loaded_files = local_doc_qa.one_knowledge_add(
                vs_path, files, one_conent, one_content_segmentation, sentence_size
            )
        if len(loaded_files):
...
def add_vs_name(vs_name, vs_list, chatbot):
    if vs_name in vs_list:
        ...
        chatbot = chatbot + [[None, vs_status]]
        return (
            gr.update(visible=True, choices=[vs_name] + vs_list, value=vs_name),
            [vs_name] + vs_list,
            gr.update(visible=False),
            gr.update(visible=False),
            gr.update(visible=True),
...
with gr.Blocks(css=block_css) as demo:
    vs_path, file_status, model_status, vs_list = (
        gr.State(os.path.join(VS_ROOT_PATH, vs_list[0]) if len(vs_list) > 1 else ""),
        gr.State(""),
                    ...
                    with file2vs:
                        ...
                        with gr.Tab("上传文件夹"):
                            folder_files = gr.File(
                                label="添加文件", file_count="directory", show_label=False
                            )
                            load_folder_button = gr.Button("上传文件夹并加载知识库")
                    vs_refresh.click(fn=refresh_vs_list, inputs=[], outputs=select_vs)
                    vs_add.click(
                        fn=add_vs_name,
    ...
    with gr.Tab("知识库测试 Beta"):
        with gr.Row():
            ...
            with gr.Column(scale=5):
                mode = gr.Radio(
                    ["知识库测试"], label="请选择使用模式", value="知识库测试", visible=False  # "知识库问答",
                )
