...
"""
-----------------------------------------------------
   File Name：     vedisClient.py
   Description :   封装Vedis相关操作,Vedis是嵌入式，不需要像Redis需要单独部署服务器
   Author :        
   date：          2022/12/2
------------------------------------------------------
   Change Activity:
                   2019/08/09: 封装Vedis相关操作
...
class VedisClient(object):
    ...
    def test(self):
        log = LogHandler("vedis_client")
        try:
            self.getCount()
        except TimeoutError as e:
            log.error(f"vedis connection time out: {str(e)}", exc_info=True)
            return e
        except (ConnectionError, ResponseError) as e:
            log.error(f"vedis connection error: {str(e)}", exc_info=True)
            return e
