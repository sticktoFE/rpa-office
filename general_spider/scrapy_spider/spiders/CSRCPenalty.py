...
import time
import scrapy
from general_spider.scrapy_spider.items import CSRCPenaltyItem
...
class CSRCPenaltySpider(SeleniumSpider):
    ...
    custom_settings = {
        "ITEM_PIPELINES": {
            "general_spider.scrapy_spider.pipelines.CSRCPenaltyPipeline": 301
        }
    }
    """
        开始发起请求，记录页码
    ...
    def parse(self, response):
        ...
        if meta["page_num"] != -1:  # -1表示没有下一页了
            ...
            articles = response.xpath(
                '//div[@id="codeId_list"]//table/tbody/tr[position()>1]'
            )
            i = 0
    ...
    def selenium_func(self, request):
        ...
        if meta["purpose"] == "next":
            next_page = waitForXpath(self.browser, "//a[text()='下一页']")
        ...
        # 此处等待翻页完成，显示当前页码已经是正确的页码，但其实页面内容却没有加载完成
        waitForXpath(
            self.browser,
