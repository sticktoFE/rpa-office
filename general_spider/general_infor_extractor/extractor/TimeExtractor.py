...
class TimeExtractor:
    ...
    def extractor(self, element: HtmlElement, publish_time_xpath: str = "") -> str:
        publish_time_xpath = publish_time_xpath or config.get("publish_time", {}).get(
            "xpath"
        )
        publish_time = (
            self.extract_from_user_xpath(
                publish_time_xpath, element
            )  # 用户指定的 Xpath 是第一优先级
    ...
    def extract_from_user_xpath(
        self, publish_time_xpath: str, element: HtmlElement
    ) -> str:
