...
from general_spider.general_infor_extractor.utils import (
    ...
    pad_host_for_images,
    config,
    get_high_weight_keyword_pattern,
)
...
class ContentExtractor:
    ...
    def extract(
        ...
        try:
            ...
            selector_source_code = unescape(
                etree.tostring(selector, encoding="utf-8").decode()
            )
            print(selector_source_code)
            print(error)
        ...
        self.calc_new_score()
        result = sorted(
            self.node_info.items(), key=lambda x: x[1]["score"], reverse=True
        )
        return result
