...
class GifSplashScreen(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # 反锯齿
        currentPM = currentPM.scaled(display_rect.size(), Qt.IgnoreAspectRatio, Qt.SmoothTransformation)
        currentPMRect = currentPM.rect()
        if currentPMRect.intersects(event.rect()):
            painter = QPainter(self)
            painter.setRenderHints(QPainter.SmoothPixmapTransform|QPainter.Antialiasing)  # 消锯齿
            painter.translate(self.backgroud_movie_zoomout_x/2, 0) #x方向如果缩小，即往左边减少10，开始画图x方向位置为 10/2 = 5
    ...
    def deleteItemsOfLayout(self, xlayout):
        if xlayout is not None and (type(xlayout) is QVBoxLayout or type(xlayout) is QHBoxLayout):
            while xlayout.count():
