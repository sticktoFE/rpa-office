...
class MMenu(QMenu):
    ...
    def init_menu(self):
        ...
        self.addAction(QIcon(f":icon/0{randint(1,8)}.ico"), '选项',
        ...
        # self.addSeparator()
        self.addAction(QIcon(f":icon/0{randint(1,8)}.ico"), '截图识别信息',
                       partial(self.show_signal.emit, "self.screen_shot_info()"))
        self.addSeparator()
        self.addAction(
            QIcon(f":icon/0{randint(1,8)}.ico"), '信息综合获取', self.displayInfoFetch)
        self.addSeparator()
        self.addAction(self.get_icon(), '暂停扫描', partial(
            self.show_signal.emit, "self.pauseScan()"))
        self.addSeparator()
        self.addAction(self.get_icon(), '关闭', partial(
            self.show_signal.emit, "self.close()"))
    ...
    def get_icon(self):
        ...
        painter.drawText(QRect(0, 0, 16, 16), Qt.AlignCenter,  # type: ignore
