...
def get_pos_contents(
    out_info, from_source="ocr", draw_img=None, table_pos=[0, 0, 0, 0]
):
    ...
    info_list = out_info
    if from_source == "structure-table":
        info_list = out_info["boxes"]
    elif from_source == "ocr":
        info_list = out_info[0]
        # [ [[[[46.0, 28.0], [123.0, 26.0], [124.0, 52.0], [47.0, 54.0]], ('xxxxxxxxx', 0.9719049334526062)],...]
    ...
    for i, one_item in enumerate(info_list):
        if from_source == "ocr":
        ...
        elif from_source == "structure-other":
            ...
            ocr_content = (
                out_info["rec_res"][i][0] if i < len(out_info["rec_res"]) else " "
            )
        # 对识别的图片区域通过画矩形标识出来
        if draw_img is not None:
            ...
            left_top_x = int(left_top_x + table_pos[0])
            left_top_y = int(left_top_y + table_pos[1])
            right_bottom_x = int(right_bottom_x + table_pos[0])
            right_bottom_y = int(right_bottom_y + table_pos[1])
            cv2.rectangle(
                draw_img,
        ...
        temp_list.append(
            [left_top_x, left_top_y, right_bottom_x, right_bottom_y, ocr_content]
        )
    return temp_list


# 一下效果并不好，尽量用paddle-structure的，或者后期再探讨
""" 
    对识别结果进行格式化调整
"""
...
def get_x_code_value(temp_list):
    ...
    # 最终生成一个以横坐标x值（合并后的）为列名的空PD
    return (
        pd.DataFrame(columns=sorted(set(final_column_dict.values()))),
        final_column_dict,
    )  # set具有顺序随机性，所以要相比上面再排一次序
