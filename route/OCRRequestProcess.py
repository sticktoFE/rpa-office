...
from model.OCRServer import POCRServerProcess
...
class OCRRequestProcess:
    def __init__(self, **kwargs):
        # 进程通信间的变量
        self.client_end_conn, self.server_end_conn = Pipe()
        # 开一个守护进程来作为服务，运行在整个程序期间
        self.p = POCRServerProcess(
            self.server_end_conn,
            name="anne",
            daemon=True,  # 一定要在p.start()前设置,设置p为守护进程,禁止p创建子进程,并且父进程代码执行结束,p即终止运行
        )
        self.p.start()
    ...
    def ocr(self, cv2_img):
        ...
        returnResult = self.client_end_conn.recv()  # 只要服务端不关闭，就一直阻塞等待返回的消息
        returnResult = [
            str(line[1][0]) if len(line) > 0 else "" for line in returnResult[0]
        ]
        # time.sleep(10)  # 制造阻塞
        ...
        for region in result_sorted:
            ...
            if (
                ...
                result_pos_content.extend(
                    get_pos_contents(
                        region["res"], from_source="structure", draw_img=cv2_img_c
                    )
                )
                # **1
