...
class ADB:
    ...
    def _check_target(cls, cmd, target):
        if target is None and all(
            safe_commands not in cmd for safe_commands in cls.COMMANDS_WITHOUT_TARGETS
        ):
    ...
    def get_version(cls):
        ...
        if output is None or len(output) < 1:
            cls.LOGGER.warning(
                "No version found. Check adb is on path %s.", ADB._adb_path
            )
            return None
        ...
        try:
            ...
            cls.LOGGER.exception("Unexpected output %s caused %s", output, err)
    ...
    def get_devices(cls):
        ...
        try:
            ...
            cls.LOGGER.exception("Exception being translated to PermissionsError")
    ...
    def reboot_device(self, mode):
        ...
        if mode not in (self.REBOOT_RECOVERY, self.REBOOT_BOOTLOADER):
            raise self.BadCall("mode must be REBOOT_RECOVERY/REBOOT_BOOTLOADER")
    ...
    def forward_socket(self, local=None, remote=None):
        ...
        if local is None or remote is None:
            return None
        return self._output_if_no_error(self.run_cmd(["forward", local, remote]))
...
class ADBCMD(ADB):
    ...
    def __init__(self, adb_path=r"D:\leichui\platform-tools\adb.exe"):
        super().__init__(adb_path)
        # set ADB path, using a couple of popular addresses.
        try:
            self.set_adb_path(adb_path)
        except ADB.BadCall:
            self.set_adb_path("$PREFIX/usr/bin/adb")
    ...
    def swipe(self, x1=500, y1=1200, x2=500, y2=600):
        """滑动屏幕"""
        self.shell_command(f"input swipe {x1} {y1} {x2} {y2}")
    ...
    def click_power(self):
        """点击电源键"""
        self.shell_command("input keyevent 26")
    ...
    def start_app(self, app_activity):
        self.shell_command(f"am start -n {app_activity}")
