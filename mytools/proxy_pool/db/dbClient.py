...
from mytools.proxy_pool.util.six import urlparse, withMetaclass
...
class DbClient(withMetaclass(Singleton)):
    ...
    def parseDbConn(cls, db_conn):
        ...
        cls.save_loc = db_conf.netloc
        print(f'数据库为{cls.db_type},保存地址为{cls.save_loc}')
        cls.db_host = db_conf.hostname
        if cls.db_type == "VEDIS":
            cls.db_port = None
    ...
    def __initDbClient(self):
        ...
        if self.db_type == "REDIS":
            ...
            __type = "ssdbClient"
        elif self.db_type == "VEDIS":
            __type = "vedisClient"
            self.client = getattr(__import__(
                __type), f"{self.db_type.title()}Client")(self.save_loc)
            return
        ...
        self.client = getattr(__import__(__type), f"{self.db_type.title()}Client")(
            host=self.db_host, port=self.db_port, username=self.db_user, password=self.db_pwd, db=self.db_name)
