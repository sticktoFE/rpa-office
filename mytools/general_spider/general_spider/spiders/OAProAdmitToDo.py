...
class OAProAdmitToDoSpider(SeleniumSpider):
    ...
    def parse(self, response):
        ...
        if meta["page_num"] != -1: # -1表示没有下一页了
            ...
            if meta["page_num"] < self.settings.get('MAX_PAGE'):  # 不超过3页
                ...
                yield scrapy.Request(url=current_url, meta=meta, callback=self.parse, dont_filter=True)
    ...
    def parse_article(self, item,response):
        ...
        # 背景后面的附件名称
        content_attachs = content.xpath("./tr[8]/td[3]//ul[@class='ant-list-items wapper_sd-draggable_common']/li[@class='ant-list-item actionlist_sd-attachment_common list_sd-attachment_common item_sd-draggable_common']")
        ...
        if len(content_attachs)>0:
            content_attachs_downloads = waitForXpath(
                    ...
                    timeout=self.timeout,is_elements=True
                )
            for submit_download in content_attachs_downloads:
                submit_download.click()
                time.sleep(2)
        # 相关文件下载
        relate_content_attachs = content.xpath("./tr[13]/td[2]//ul[@class='ant-list-items wapper_sd-draggable_common']/li[@class='ant-list-item actionlist_sd-attachment_common list_sd-attachment_common item_sd-draggable_common']")
