...
from PySide6.QtGui import QCursor, QGuiApplication,QPixmap, QPainter, QIcon, QFont, QColor
...
from .SMaskLayer import MaskLayer
from .SAIFinder import Finder
from .SRollScreenshot import RollScreenshot
from pynput.mouse import Controller
# 导入图片资源以便使用
import mytools.AIScreenshot.imagefiles.jamresourse_rc
class SMainLayer(QLabel):  # 区域截图功能
    ...
    def init_slabel_ui(self): 
        ...
        self.rollshot_botton.setGeometry(self.freeze_img_botton.x() + self.freeze_img_botton.width(), 0, 40, 35)
        ...
        self.confirm_botton.setGeometry(self.rollshot_botton.x() + self.rollshot_botton.width(), 0, 60, 35)
        self.confirm_botton.setIcon(QIcon(":/screenshot.png"))
        self.confirm_botton.clicked.connect(lambda: self.cutpic(3))
        self.botton_box.resize(self.ssrec_botton.width() + self.save_botton.width()  + self.rollshot_botton.width() + self.freeze_img_botton.width() + self.confirm_botton.width(),self.confirm_botton.height())
    ...
    def search_in_which_screen(self):
        ...
        for i in screens:
            ...
            if mousepos[0] in range(rect[0],rect[0]+rect[2]) and mousepos[1]in range(rect[1],rect[1]+rect[3]):
                secondscreen = i
                break
        print("t", self.x(), globalvar.get_var("SCREEN_WIDTH"),QApplication.primaryScreen().geometry(),secondscreen.geometry(),mousepos)
        return secondscreen
    # 按键按下,每按一个键触发一次
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:  # 按下了左键
            ...
            mx = (x1 + x0) // 2
            # 以下为判断点击在哪里
            if not self.finding_rect and (self.x0 - 8 < mouse_posx < self.x0 + 8) and (my - 8 < mouse_posy < my + 8 or y0 - 8 < mouse_posy < y0 + 8 or y1 - 8 < mouse_posy < y1 + 8):
                self.move_x0 = True
                r = 1
            elif not self.finding_rect and (self.x1 - 8 < mouse_posx < self.x1 + 8) and (my - 8 < mouse_posy < my + 8 or y0 - 8 < mouse_posy < y0 + 8 or y1 - 8 < mouse_posy < y1 + 8):
                self.move_x1 = True
                r = 1
            elif not self.finding_rect and (self.y0 - 8 < mouse_posy < self.y0 + 8) and (mx - 8 < mouse_posx < mx + 8 or x0 - 8 < mouse_posx < x0 + 8 or x1 - 8 < mouse_posx < x1 + 8):
                self.move_y0 = True
            elif not self.finding_rect and self.y1 - 8 < mouse_posy < self.y1 + 8 and (mx - 8 < mouse_posx < mx + 8 or x0 - 8 < mouse_posx < x0 + 8 or x1 - 8 < mouse_posx < x1 + 8):
                self.move_y1 = True
            elif not self.finding_rect and (x0 + 8 < mouse_posx < x1 - 8) and (y0 + 8 < mouse_posy < y1 - 8): 
                self.move_rect = True
                ...
                if self.x1 == -50:
                    ...
                    self.y1 = mouse_posy
                if r:  # 判断是否点击在了对角线上
                    if (self.y0 - 8 < mouse_posy < self.y0 + 8) and (x0 - 8 < mouse_posx < x1 + 8):
                        self.move_y0 = True
                        # print('y0')
                    elif self.y1 - 8 < mouse_posy < self.y1 + 8 and (x0 - 8 < mouse_posx < x1 + 8):
                        self.move_y1 = True
                        # print('y1')
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.finding_rect:  
            ...
            my = (maxy + miny) // 2
            mx = (maxx + minx) // 2  # 取中间值
            if ((minx - 8 < self.mouse_posx < minx + 8) and (miny - 8 < self.mouse_posy < miny + 8)) or \
                        ((maxx - 8 < self.mouse_posx < maxx + 8) and (maxy - 8 < self.mouse_posy < maxy + 8)):
                self.setCursor(Qt.SizeFDiagCursor)
            elif ((minx - 8 < self.mouse_posx < minx + 8) and (maxy - 8 < self.mouse_posy < maxy + 8)) or \
                        ((maxx - 8 < self.mouse_posx < maxx + 8) and (miny - 8 < self.mouse_posy < miny + 8)):
                self.setCursor(Qt.SizeBDiagCursor)
            elif (self.x0 - 8 < self.mouse_posx < self.x0 + 8) and (
                    my - 8 < self.mouse_posy < my + 8 or miny - 8 < self.mouse_posy < miny + 8 or maxy - 8 < self.mouse_posy < maxy + 8):
                self.setCursor(Qt.SizeHorCursor)
            elif (self.x1 - 8 < self.mouse_posx < self.x1 + 8) and (
                    my - 8 < self.mouse_posy < my + 8 or miny - 8 < self.mouse_posy < miny + 8 or maxy - 8 < self.mouse_posy < maxy + 8):
                self.setCursor(Qt.SizeHorCursor)
            elif (self.y0 - 8 < self.mouse_posy < self.y0 + 8) and (
                    mx - 8 < self.mouse_posx < mx + 8 or minx - 8 < self.mouse_posx < minx + 8 or maxx - 8 < self.mouse_posx < maxx + 8):
    ...
    def choice(self):  
        ...
        if dx < self.botton_box.width() + 10:
            botton_boxw = min(self.x0, self.x1) - self.botton_box.width() - 5 if max(self.x1, self.x0) + self.botton_box.width() > x else max(self.x1, self.x0) + 5
        elif self.x1 > self.x0:
            botton_boxw = self.x1 - self.botton_box.width() - 5
        if dy < self.botton_box.height() + 105:
            botton_boxh = min(self.y0, self.y1) - self.botton_box.height() - 5 if max(self.y1, self.y0) + self.botton_box.height() + 20 > y else max(self.y0, self.y1) + 5
    ...
    def cutpic(self, save_as=0):
        ...
        if save_as:
            if save_as == 1:
                path, l = QFileDialog.getSaveFileName(self, "保存为", QStandardPaths.writableLocation(QStandardPaths.PicturesLocation), "img Files (*.PNG *.jpg *.JPG *.JPEG *.BMP *.ICO);;all files(*.*)")
                if path:
                ...
                worker = Worker(get_ocr_info,image_convert.pixmap2cv(self.final_get_img))
    ...
    def clear_and_hide(self):
        self.hide()
        self.botton_box.hide()
        worker = Worker(self.clear_and_hide_thread) # Any other args, kwargs are passed to the run function
