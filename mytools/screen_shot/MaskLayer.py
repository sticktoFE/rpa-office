...
class MaskLayer(QLabel):
    ...
    def paintEvent(self, e):
        ...
        if self.parent.on_init:
            print("正在初始化，所以masklayer不paint，直接return")
            return
        painter = QPainter(self)
        painter.setRenderHint(QPainter.Antialiasing)
        # 正常显示选区
        rect = QRect(
            min(self.parent.x0, self.parent.x1),
        ...
        painter.drawPoint(
            QPoint(
                ...
                + abs(self.parent.x1 - self.parent.x0) // 2,
                self.parent.y0,
            )
        )
        painter.drawPoint(
        ...
        painter.drawText(
            x,
            y,
            f"{abs(self.parent.x1 - self.parent.x0)}x{abs(self.parent.y1 - self.parent.y0)}",
        )
        ...
        painter.drawText(
            enlarge_box_x,
            enlarge_box_y - 8,
            f"({self.parent.mouse_posx}x{self.parent.mouse_posy})",
        )
        try:
            ...
            rpixpainter.end()
            larger_pix = rpix.copy(
                self.parent.mouse_posx, self.parent.mouse_posy, 120, 120
            ).scaled(
                120 + self.parent.tool_width * 10, 120 + self.parent.tool_width * 10
            )
            pix = larger_pix.copy(
                larger_pix.width() // 2 - 60, larger_pix.height() // 2 - 60, 120, 120
            )
