...
def split(img, split_num = 0 ,split_length = 0 ,axis=1):
    ...
    if split_num == 0 and split_length >0:
        split_num = width//split_length+1 if axis == 0 else height//split_length+1
    if split_num > 0 and split_length == 0:    
    ...
    index_list = [index for index in range(1,row_histogram.shape[0]-1) if row_histogram[index] ==  row_histogram[index - 1] and row_histogram[index] ==  row_histogram[index + 1] ]
    ...
    for i, idx in enumerate(idx_list):
        if i == 0:
            img_list.append(img[:, :idx, :]) if axis == 0 else img_list.append(img[:idx, :, :])
        else:
            img_list.append(img[:, idx_list[i-1]:idx, :]) if axis == 0 else img_list.append(img[idx_list[i-1]:idx,:, :]) 
