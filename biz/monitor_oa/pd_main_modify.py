...
class ProductMainModify:
    ...
    def login(self):
        ...
        self.browser.get(
            "http://plm.zybank.com.cn/#/system-tool/main-product-management"
        )  # 填写表的地址
        ...
        submit = waitForXpath(
            system_tool,
            "//div[@class='menu_ware nest-menu' and @indexpath='/system-tool/main-product-management/main-product-management']",
            timeout=self.timeout,
        )
        submit.click()
    ...
    def add_product(self, product_info: dict):
        ...
        waitForXpath(
            self.browser,
            "//div[@class='_button' and contains(text(),'主产品新增')]",
            timeout=self.timeout,
        ).click()
        # 循环更新主产品信息
        for key, value in product_info.items():
            ...
            if key == "产品分类":
                ...
                waitForXpath(
                    self.browser,
                    f'//div[div[contains(@class,"font12 textRight mr8") and @style="white-space: nowrap; width: 130px;" and contains(text(),"产品分类")]/following-sibling::div[1]//input[@type="text" and @readonly="readonly" and @autocomplete="off" and @placeholder="请选择" and @class="el-input__inner"]',
                    timeout=self.timeout,
                ).click()
                # 选择下拉菜单里的内容
                parent_drop_select = waitForXpath(
                ...
                for product_class in value.split("/"):
                    drop_select_radio = waitForXpath(
                        parent_drop_select,
                        f'.//div[@class="el-scrollbar el-cascader-menu" and @role="menu"]//ul[@class="el-scrollbar__view el-cascader-menu__list"]/li[contains(@class,"el-cascader-node")]/span[text()="{product_class.strip()}"]',
                        timeout=self.timeout,
                    )
                    drop_select_radio.click()
            # n）以下为需要更新的信息
...
if __name__ == "__main__":
    sm = ProductMainModify(userID="000216", passwd="abcd@1234")
    sm.update_add_product_info(
        file_path=r"D:\temp\retail_yinhangka_product_lis.txt", to_add=True
    )  # ProductChildModify  ProductMainModify
    # 添加子产品信息
    # sm_child = ProductChildModify(userID='000216',passwd='abcd@1234')
    # sm_child.parse()  #ProductChildModify  ProductMainModify
